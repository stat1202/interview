...
- 렉시컬 환경이란
    ...
    환경 레코드는 실행할 스코프 범위 안에 있는 변수와 함수를 프로퍼티로 저장하는 객체입니다.
...
- 제네레이터란
    ...
    function*로 만들어지는 제네레이터 함수를 통해 만들 수 있으며, 제네레이터 객체를 반환하고
    
    next()를 수행할 떄마다 yield를 하나씩 반환합니다.
    
- 이터레이터와 이터레이터 프로토콜
    
    이터레이터 프로토콜은 순회 가능한 데이터 콜렉션을  만들기 위해 ES6에서 도입된 규칙입니다. 이터레이터 프로토콜을 준수한 객체를 이터레이터라고 합니다.
...
- state와 props의 차이를 설명해주세요
    ...
    state는 컴포넌트에서 사용하고 관리할 수 있는 상태값으로 setState로 값 변경 시 렌더링이 일어납니다.
